cc.Class({
    ...
    dash() {
        ...
        cc.log(`[Dash] Khóa vị trí mục tiêu tại: (${playerPos.x.toFixed(2)}, ${playerPos.y.toFixed(2)})`);
            ...
            cc.tween(boss)
                ...
                .call(() => {
                    ...
                    players.forEach(p => {
                        ...
                        if (dist < 100) {
                            let playerScript = p.getComponent("Player");
                            if (playerScript?.takeDamage) {
                                playerScript.takeDamage(this.damage);
                                cc.log(`[Dash] Gây damage ${this.damage} cho player tại khoảng cách ${dist.toFixed(2)}`);
                            } else {
                                cc.warn("[Dash] Không tìm thấy hàm takeDamage trong player");
                            }
                        } else {
                            cc.log(`[Dash] Player cách xa ${dist.toFixed(2)}, không gây damage`);
                        }
                    });
                })
                .start();

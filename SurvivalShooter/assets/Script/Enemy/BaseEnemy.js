cc.Class({
  ...
  onDie() {
    ...
    if (this.expPrefab) {
      ...
      if (expScript) {
        ...
        // FIX: Truyền node thay vì component
        expScript.targetPlayer = this.target; // this.target là node
        // Hoặc nếu muốn chắc chắn:
        // expScript.targetPlayer = this.player ? this.player.node : this.target;
      }
    }
  ...
  setSpriteDirection(facingLeft) {
    ...
    if (this.originalFacingLeft) {
      // Original faces left
      this.node.scaleX = facingLeft
        ? Math.abs(this.originalScaleX)
        : -Math.abs(this.originalScaleX);
    } else {
